...
export default function CardRecommendationPage() {
  ...
  type FilterCategory = "benefits" | "brands" | "monthlySpend" | "annualFee";
  ...
  const benefitOptions = [
    ...
    {
      value: "TRANSPORT_AUTOMOBILE",
      label: "교통/자동차",
      icon: <Car className="h-4 w-4" />,
    },
    {
      value: "TRAVEL_AIRLINE",
      label: "여행/항공",
      icon: <Plane className="h-4 w-4" />,
    },
    {
      value: "CULTURE_LEISURE",
      label: "문화/레저",
      icon: <Tv className="h-4 w-4" />,
    },
    {
      value: "LIVING_SERVICES",
      label: "생활서비스",
      icon: <Home className="h-4 w-4" />,
    },
    {
      value: "EDUCATION_CHILDCARE",
      label: "교육/육아",
  ...
  return (
    <main className="min-h-screen bg-mesh">
      ...
      <Link href="/me">
        <div className="absolute top-[1cm] left-[5cm] flex h-10 w-20 cursor-pointer items-center justify-center rounded-lg bg-gradient-to-r from-blue-500 bg-blue-400 text-white text-xl font-semibold shadow-md hover:shadow-lg hover:scale-105 transition-all duration-300">
          유저정보
        </div>
      </Link>
      ...
      <div className="container mx-auto py-8 px-4">
        <div className="flex flex-col items-center justify-center mb-10 pt-6 animate-fade-in">
          ...
          <p className="text-muted-foreground text-center max-w-md">
            나에게 맞는 최적의 카드를 찾아보세요. 혜택, 브랜드, 사용액에 따라
            맞춤형 카드를 추천해드립니다.
          </p>
        </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
                  {benefitOptions.map((option) => (
                    <FilterOption
                      ...
                      onToggle={(value) =>
                        handleFilterToggle("benefits", value)
                      }
                    />
                  ))}
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {monthlySpendOptions.map((option) => (
                    <FilterOption
                      ...
                      onToggle={(value) =>
                        handleFilterToggle("monthlySpend", value)
                      }
                    />
                  ))}
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {annualFeeOptions.map((option) => (
                    <FilterOption
                      ...
                      onToggle={(value) =>
                        handleFilterToggle("annualFee", value)
                      }
                    />
                  ))}
                </div>
